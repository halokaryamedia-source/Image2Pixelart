<script lang="ts">
	...
	import { createProjectPdfInBackground } from '$lib/export/pdf-client';
	...
	async function uploadImage(event: Event) {
		...
		input.value = '';
		if (!file || uploading || !editable) return;
		uploading = true;
		localError = null;
		try {
			const next = await convertProjectImage(project, file, {
				suggestPalette: true,
				applyPalette: true,
				applyCells: true,
				replaceSource: true
			...
			onChange(next);
			if (onSourceImageChange) await onSourceImageChange(file, next);
		} catch (caught) {
			localError = caught instanceof Error ? caught.message : 'Gambar tidak dapat diproses.';
		} finally {
			uploading = false;
		}
	}
	...
	function exportOther(kind: 'png-grid' | 'materials' | 'matrix' | 'project') {
		if (kind === 'png-grid') {
			exporting = 'PNG + grid';
			void createProjectPng(project, true).then((blob) => downloadBlob(blob, `${safeFileName(project.name)}-grid.png`)).catch(() => (localError = 'PNG + grid tidak dapat dibuat.')).finally(() => (exporting = null));
			return;
		}
		...
		<div class="start-layer">
			<div class="start-card">
				...
				<div class="start-actions">
					<label class="primary-shell">{uploading ? 'Memproses gambar…' : 'Upload gambar'}<input class="sr-only" type="file" accept="image/png,image/jpeg,image/webp" onchange={uploadImage} disabled={uploading || !editable} /></label>
					<button class="secondary-shell" type="button" onclick={() => (startedBlank = true)} disabled={!editable}>Mulai build kosong</button>
				</div>
				<small>Setelah upload, hasil pixel dibuat otomatis. Pengaturan lanjutan tetap tersedia melalui panel Editor.</small>
			</div>
		</div>
	{/if}
...
<style>
	...
	@media(max-width:900px){.player-meta{left:80px}.player-status{right:102px}.more-tools{bottom:62px}}@media(max-width:620px){.player-meta{display:none}.finish-grid{grid-template-columns:1fr}.player-status{display:none}.start-actions{flex-direction:column}.start-actions>*{width:100%}}
</style>