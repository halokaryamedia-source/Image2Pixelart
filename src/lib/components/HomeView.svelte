<script lang="ts">
	...
	function baseName(fileName: string) { return fileName.replace(/\.[^.]+$/, '').replace(/[-_]+/g, ' ').trim().slice(0, 120) || 'Proyek mosaic'; }
	function roleLabel(role: CloudProjectSummary['role']) { return role === 'owner' ? 'Pemilik' : role === 'editor' ? 'Editor' : 'Hanya lihat'; }
	function chooseImage(file?: File) {
		if (!file) return; fileError = '';
		if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) { fileError = 'Gunakan PNG, JPEG, atau WebP.'; return; }
		if (file.size > MAX_IMAGE_BYTES) { fileError = 'Ukuran gambar melebihi 20 MB.'; return; }
		if (previewUrl) URL.revokeObjectURL(previewUrl);
		selectedFile = file; previewUrl = URL.createObjectURL(file); name = baseName(file.name);
	}
	...
	async function submit(event: SubmitEvent) {
		...
		try { await onCreate({ name, widthMm: cmToMm(widthCm), heightMm: cmToMm(heightCm), cellMm: cmToMm(cellCm), mode, file: selectedFile ?? undefined }); }
...
<header class="topbar">
	<a class="brand" href="/" aria-label="MIVUBI Mosaic Plan, beranda"><img src="/mivubi-logo.png" alt="" width="36" height="36" /><span><strong>MIVUBI</strong><small>PIXEL MOSAIC PLANNER</small></span></a>
	...
	<label class="secondary-button"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h6l2 2h9v10h-17z" /></svg>Buka file proyek<input class="sr-only" type="file" accept=".json,.pixelgrid.json,application/json" onchange={importFile} /></label>
</header>

<main>
	<section class="intro"><p class="eyebrow">MIVUBI · PIXEL MOSAIC PLANNER</p><h1>Dari gambar ke <em>grid presisi.</em></h1><p>Siapkan ukuran dan gambar, lalu lanjutkan ke editor dengan grid yang sesuai ukuran fisik.</p></section>
	<section class="quick-start" aria-label="Mulai proyek baru">
		<form class="setup-card" onsubmit={submit}>
			<div class="mode-tabs" role="tablist" aria-label="Cara memulai proyek">
				<button class:active={mode === 'image'} type="button" role="tab" aria-selected={mode === 'image'} onclick={() => (mode = 'image')}><span>▧</span>Dari gambar</button>
				<button class:active={mode === 'blank'} type="button" role="tab" aria-selected={mode === 'blank'} onclick={() => (mode = 'blank')}><span>▦</span>Canvas kosong</button>
			</div>
			{#if mode === 'image'}
				<label class:has-file={!!selectedFile} class="dropzone" ondragover={(event) => event.preventDefault()} ondrop={dropImage}>
					...
					<strong>{selectedFile ? selectedFile.name : 'Unggah gambar atau seret ke sini'}</strong>
		...
		<aside class="visual-card" aria-label="Preview ukuran grid">
			<div class="preview-heading"><strong>Preview · {validation.valid ? `${validation.columns} × ${validation.rows}` : '—'}</strong><span>{mode === 'image' && selectedFile ? 'Gambar siap diproses' : 'Grid fisik'}</span></div>
