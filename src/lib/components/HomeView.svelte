...
<main>
	<section class="intro"><p class="eyebrow">MIVUBI · PIXEL MOSAIC PLANNER</p><h1>Dari gambar ke <em>grid presisi.</em></h1><p>Siapkan ukuran dan gambar, lalu buat grid produksi dalam hitungan detik.</p></section>
	<section class="quick-start" aria-label="Mulai proyek baru">
		...
		<aside class="visual-card" aria-label="Preview ukuran grid">
			...
			<p>{mode === 'image' ? 'Gambar akan dianalisis lokal dan dibuat menjadi suggestion 8 warna.' : 'Canvas akan dimulai kosong tanpa tile atau palet.'}</p>
		</aside>
	</section>
	<section class="projects" id="projects">
		<div class="projects-heading"><div><p class="eyebrow">PROYEK CLOUD</p><h2>Lanjutkan pekerjaan</h2></div><span>{projects.length} proyek terhubung ke perangkat ini</span></div>
		{#if !ready}<div class="empty">Memuat proyek cloud…</div>{:else if projects.length === 0}<div class="empty"><strong>Belum ada proyek cloud.</strong><span>Proyek pertamamu akan muncul di sini dan tersimpan otomatis ke Neon.</span></div>{:else}
			<div class="project-grid">{#each projects as project}<article><button class="project-open" type="button" onclick={() => onOpen(project)}><div class="mini-mosaic"><CloudProjectThumbnail {project} /></div><div class="project-copy"><strong>{project.name}</strong><span>{project.columns} × {project.rows} grid · {project.palette.length} warna · {project.role}</span><small>{project.deletedAt ? 'Di tempat sampah' : `Diubah ${new Date(project.updatedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}`}</small><b>{project.deletedAt ? 'Buka status' : 'Lanjutkan'} <i>→</i></b></div></button><button class="menu-trigger" type="button" aria-label={`Menu proyek ${project.name}`} aria-expanded={openMenu === project.id} onclick={() => (openMenu = openMenu === project.id ? null : project.id)}>⋮</button>{#if openMenu === project.id}<div class="project-menu"><button type="button" onclick={() => { openMenu = null; if (confirm(`Pindahkan proyek “${project.name}” ke tempat sampah selama 7 hari?`)) onDelete(project.id); }} disabled={project.role !== 'owner' || !!project.deletedAt}>Hapus proyek</button></div>{/if}</article>{/each}</div>
		{/if}
	</section>
</main>
