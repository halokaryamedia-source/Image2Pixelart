...
<div class="editor-shell">
	...
	<div class="editor-body">
		<aside class="left-panel">
			...
			{:else}
				<section><p class="section-number">02 / PALETTE HEX</p><h2>{project.palette.length} warna aktif</h2><p class="helper">Warna bersifat lokal untuk proyek ini. Edit HEX akan langsung mewarnai ulang semua sel pada slot yang sama.</p>
		...
		<aside class="right-panel"><p class="section-number">03 / PRODUKSI</p><h2>Export blueprint</h2><div class="production-total"><span>TILE TERISI</span><strong>{filledCount.toLocaleString('id-ID')}</strong><small>{emptyCount.toLocaleString('id-ID')} sel kosong · {project.palette.length} warna</small></div><button class="export-primary" type="button" onclick={exportPdf} disabled={!!exporting}><span><b>PDF</b><small>Overview + panel 24 × 24</small></span><i>{exporting === 'PDF' ? '…' : '↓'}</i></button><div class="export-grid"><button type="button" onclick={() => exportPng(false)} disabled={!!exporting}><b>PNG</b><small>Transparan</small></button><button type="button" onclick={() => exportPng(true)} disabled={!!exporting}><b>PNG + Grid</b><small>Preview label</small></button><button type="button" onclick={exportMaterialsCsv}><b>CSV material</b></button><button type="button" onclick={exportMatrixCsv}><b>CSV matriks</b></button><button type="button" onclick={exportProject}><b>Project</b></button></div><div class="material-summary"><div><span>WARNA HEX</span><span>TILE</span></div>{#each project.palette as entry}<p><span><i style={`--color:${entry.hex}`}></i>{entry.hex}</span><b>{counts[entry.slot]?.toLocaleString('id-ID')}</b></p>{/each}<footer><span>Total terisi</span><b>{filledCount.toLocaleString('id-ID')}</b></footer></div></aside>
	</div>
