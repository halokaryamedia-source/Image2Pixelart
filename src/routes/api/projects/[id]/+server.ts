...
export const PUT: RequestHandler = async ({ request, params }) => {
	try {
		...
		const current = await db().query(
			'SELECT active_editor_device_id, editor_epoch, document FROM projects WHERE id = $1::uuid AND deleted_at IS NULL', [id]
		) as Array<{ active_editor_device_id: string | null; editor_epoch: number; document: { widthMm: number; heightMm: number; cellMm: number; columns: number; rows: number } }>;
		if (!current.length) throw new ApiError(404, 'Proyek tidak ditemukan.');
		const stored = current[0].document;
		if (
			project.widthMm !== stored.widthMm || project.heightMm !== stored.heightMm || project.cellMm !== stored.cellMm ||
			project.columns !== stored.columns || project.rows !== stored.rows
		) throw new ApiError(403, 'Ukuran Canvas file tidak dapat diubah.');
